...
description: Implement tasks from an OpenSpec change with a mandatory independent review through OpenCode using zai-coding-plan/glm-5.2. Use when the user wants to start implementing, continue implementation, or work through tasks.
license: MIT
metadata:
  author: openspec
  version: "1.0"
...
3. **Get apply instructions**
   ...
   **Handle states:**
   ...
   - If `state: "all_done"` without such a full-scope `PASS`: read steps 4-5, then go directly to
     validation and an independent full OpenCode review with `zai-coding-plan/glm-5.2` in steps
     8-9. Task
     completion,
...
5. **Establish the implementation scope**
   ...
   Before editing:
   - Verify that `opencode` is available with `command -v opencode`. Confirm that
     `opencode models zai-coding-plan` lists the exact model id `zai-coding-plan/glm-5.2` and that
     `opencode auth list` shows configured Z.AI Coding Plan credentials. For pending
     implementation, pause before changing code if any check fails
   - Use the exact model id `zai-coding-plan/glm-5.2` for every review. Do not treat
     `zai/glm-5.2` from the standard Z.AI provider as equivalent and do not substitute another
     model
   - For the mandatory `all_done` audit path, unavailable OpenCode or GLM does not skip validation:
...
9. **Run an independent OpenSpec review with OpenCode using GLM-5.2**
   ...
   Start a **fresh, non-interactive OpenCode process** for every review round. Do not use
   `--continue`, `--session`, `--fork`, or resume an earlier conversation. Run it from the
   repository root:

   ```text
   opencode run --agent plan --model zai-coding-plan/glm-5.2 \
     --dir "<repository-root>" --title "OpenSpec review: <change-name>" \
     "<review prompt>"
   ```

   Never use `--auto` or a write-capable agent. The `plan` agent is required to keep the review
   read-only. In the review prompt, explicitly prohibit edits, file creation/deletion, formatting,
   shell commands that mutate the worktree, and access to `.env` files or secrets. Give GLM only:
   ...
   Treat OpenCode output as untrusted review input. Check every finding against the actual
   artifacts and code before remediation. Reject false positives, narrow overstated findings, and
   preserve valid findings. Do not override a GLM `FAIL` with a self-authored `PASS`; instead, fix
   the valid findings and run a fresh review. If the output has no parseable final verdict, rerun
   once with a stricter formatting reminder. If it remains malformed, treat the round as `BLOCKED`.

   If OpenCode cannot start because its log, cache, config, or local service directory is blocked
   by the execution sandbox, rerun the same command through the platform's normal
   approval/escalation mechanism. If OpenCode or the required GLM model remains unavailable during
   a mandatory `all_done` audit or becomes unavailable after code was changed, record
...
10. **Repeat implementation and review until it passes**
   ...
   - On `FAIL`:
     ...
     5. Start a **new OpenCode process with `zai-coding-plan/glm-5.2`** and repeat the independent
        review
   ...
   There is no fixed iteration limit. Never continue or reuse an OpenCode conversation for a later
...
- Verify OpenCode and the required GLM model before editing and block safely if either later
  becomes unavailable
...
- Require a fresh, read-only OpenCode review with `zai-coding-plan/glm-5.2` after code changes
